...
float vibration=sin(constants.iTime*60.)*.0013;
#else
float vibration=0.;
#endif
float det=0.0;
...

{
	...
	vec3 wob=cos(dir*500.0*length(from-pth1)+(from-pth1)*250.+constants.iTime*10.)*0.0005;
	...
	for (int i=0; i<RAY_STEPS; i++) {
		if (d.x>det && totdist<3.0) {
			...
			totdist+=d.x; 
			energy=ENERGY_COLOR*(1.5+sin(constants.iTime*20.+p.z*10.))*.25;
...
void main()
{
	...
	vec2 uv = fragCoord.xy / constants.iResolution.xy*2.-1.;
	vec2 uv2=uv;
#ifdef ENABLE_POSTPROCESS
	uv*=1.+pow(length(uv2*uv2*uv2*uv2),4.)*.07;
#endif
	uv.y*=constants.iResolution.y/constants.iResolution.x;
	vec2 mouse=(constants.iMouse.xy/constants.iResolution.xy-.5)*3.;
	if (constants.iMouse.z<1.) mouse=vec2(0.);
	...
	uv2.y *= constants.iResolution.y / 360.0;
